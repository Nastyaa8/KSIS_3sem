...
#pragma comment(lib, "iphlpapi.lib") // Подключение библиотеки для работы с ICMP
...
// Определение кодов статуса ICMP
...
IP_OPTION_INFORMATION option = { 255, 255, 240, 0, 0 }; // Параметры TTL и другие
...
void Ping(const char* cHost, unsigned int Timeout, unsigned int RequestCount) {
    ...
    pIpe->Data = SendData; // Устанавливаем буфер данных SendData в структуре pIpe
    pIpe->DataSize = sizeof(SendData); // Определяем размер этого буфера в DataSize
    unsigned long ipaddr = inet_addr(cHost); // Преобразование адреса хоста в числовой формат

    // Цикл отправки ICMP-запросов
    for (unsigned int c = 0; c < RequestCount; c++) {
        ...
        if (dwStatus > 0) {
            ...
            for (int i = 0; i < dwStatus; i++) {
                ...
                // Обновляем минимальное время ответа
    ...
    // Закрытие дескриптора ICMP-сервиса и очистка сокета
    IcmpCloseHandle(hIP);
    WSACleanup();

    // Подсчет статистики и вывод результатов пинга
    if (MinMS < 0) MinMS = 0; // Если значение MinMS меньше нуля, устанавливаем его в ноль
    unsigned char* pByte = (unsigned char*)&pIpe->Address; // Создается указатель на начало байтового представления IP-адреса
...
int main(int argc, char** argv) {
    ...
    const char* par0 = argv[1]; // Хост для пинга
    int par1 = atoi(argv[2]); // Таймаут
    int par2 = atoi(argv[3]); // Количество запросов
    ...
    system("pause"); // Ожидание нажатия клавиши перед закрытием
